#!/usr/bin/env python3
"""Generate pairwise configs for coupled_v3 (Huber CD, no armadilo).

Changes from v2:
  - chamfer_huber_delta: 1.5 (robust Chamfer — caps gradient for d > δ)
  - Excludes armadilo pairs (only 9 shapes × 8 = 72 pairs)
  - Output: configs/pairwise_coupled_v3/ and output/pairwise_coupled_v3/

Usage:
    python tools/gen_pairwise_configs_v3.py
"""
from pathlib import Path
from itertools import permutations

SHAPES = ['sphere', 'bunny', 'spot', 'C', 'dragon', 'E', 'teapot', 'V', 'bob']
# NOTE: armadilo excluded
MESH_MAP = {s: f'assets/{s}.obj' for s in SHAPES}
MESH_MAP['sphere'] = 'assets/isosphere.obj'

TEMPLATE = """\
# Pairwise COUPLED v3 (Huber): {src} -> {tgt}
# coupled rev, rev_base=0.3, physics_final=0.5, huber_delta=1.5, early_stop_patience=8

input_mesh_path: "{src_mesh}"
target_mesh_path: "{tgt_mesh}"
output_dir: "output/pairwise_coupled_v3/{name}/"
...
def main():
    ...
    cfg_dir = Path('configs/pairwise_coupled_v3')
    ...
    print(f'Generated {count} configs in {cfg_dir}/')
